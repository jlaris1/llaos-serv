...
module.exports = {
                                ...
                                if(error){
                                    ...
                                    UnidadesNegocio.findById({ "_id": orden.unidad_negocio }, (error, unidad_negocio, next) =>{
                                        if(error) {
                                            ...
                                            // Logo empresa
                                            if(unidad_negocio.codigo == "LLAOS"){
                                            ...
                                            .fontSize(14)
                                            .text(unidad_negocio.razon_social, 310, 40, { align: 'right', width: 290 })
                                            .text(unidad_negocio.rfc, { align: 'right', width: 290 })
                                            ...
                                            // Datos de la empresa
                                            doc.fillColor('black')
                                            doc.text(unidad_negocio.direccion.split(",")[0], 15, 140, { align: 'left', width: 200})
                                            .text(unidad_negocio.direccion.split(",")[1], { align: 'left', width: 200})
                                            .text( "CP "  + unidad_negocio.cp + ", " + unidad_negocio.direccion.split(",")[2] + ", " + unidad_negocio.direccion.split(",")[3], { align: 'left', width: 200})
                                            ...
                                            UnidadesNegocio.populate( orden, {path: 'unidad_negocio'},  (error, orden, next) =>{
                                                if(error){
                                                    ...
                                                    respuesta.render("Compras/ordenes/ver", {
                                                        ...
                                                        articulos: articulos,
                                                        orden: orden.id,
                                                        unidad_negocio: orden.unidad_negocio.nombre,
                                                        proveedores: proveedores,
                                                        proveedor: orden.proveedor,
                    ...
                    if(error) {
                        ...
                        Usuarios.find( function(error, usuarios){
                            if(error){
                                ...
                                Camionetas.populate(oRutas, {path: 'unidad'}, (error, oRutas) => {
                                    if(error) {
                                        console.log(error);
                                    } else {
                                        Usuarios.populate(oRutas, {path: 'chofer'}, (error, oRutas) => {
                                            if(error) {
                                                console.log(error);
                                            } else {
    ...
    entradaOrdenRuta: (solicitud, respuesta) => {
        if(solicitud.session.user === undefined){
            ...
            OrdenesRuta.updateOne({"_id": solicitud.body.id_orden}, data,function(error){
                if(error){
                    console.log(error);
                } else {
                    if(solicitud.body.articulos != undefined){
                        for(let i = 0; i <= solicitud.body.articulos.length - 1; i++){
                            ...
                            var data = {
                                cantidad: parseFloat(solicitud.body.articulos[i].id_producto.cantidad) + parseFloat(solicitud.body.articulos[i].cantidad)
                            }
    
                            Productos.updateOne({"_id": solicitud.body.articulos[i].id_producto._id}, data, (error) => {
                                if(error){
                                    console.log(chalk.bgRed(error));
                                } else {
                        ...
                        respuesta.json({estatus: 'En Inventario'});
                    }
                }
            });
        }
        ...
        if(solicitud.session.user === undefined){
            ...
            OrdenesRuta.findOne({"_id": solicitud.params.id}, (error, orden) => {
                if(error){
                    ...
                    Ordenes.populate(orden.articulos, {path: "orden"}, (error, articulos) => {
                        if(error){
                            console.log(error);
                        } else {
                            Productos.populate(articulos, {path: "id_producto"}, (error, articulos) =>{
                                if(error){
                                    console.log(error);
                                } else {
                                    Proveedores.populate(articulos, {path: "id_producto.proveedor"}, (error, articulos) => {
                                        if(error){
                                            console.log(error);
                                        } else {
                                            respuesta.json(articulos)
